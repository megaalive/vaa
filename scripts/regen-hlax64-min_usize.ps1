...
$source = Join-Path $hlaxRoot "examples/interop/semasm-vaa/min_usize/min_usize.hla64"
$outAsm = Join-Path $vaaRoot "fixtures/ingest/hlax64_min_usize/candidate.asm"
...
try {
    ...
    if ($text -notmatch "global min_usize") {
        throw "emit output missing global min_usize"
    }
