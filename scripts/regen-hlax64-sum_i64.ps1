...
$source = Join-Path $hlaxRoot "examples/interop/semasm-vaa/sum_i64/sum_i64.hla64"
$outAsm = Join-Path $vaaRoot "fixtures/ingest/hlax64_sum_i64/candidate.asm"
...
try {
    ...
    if ($text -notmatch "global sum_i64") {
        throw "emit output missing global sum_i64"
    }
