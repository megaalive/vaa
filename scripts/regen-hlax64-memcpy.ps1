...
<#
.SYNOPSIS
  Regenerate fixtures/ingest/hlax64_memcpy/candidate.asm from HlaX64.
...
$source = Join-Path $hlaxRoot "examples/interop/semasm-vaa/memcpy/memcpy.hla64"
$outAsm = Join-Path $vaaRoot "fixtures/ingest/hlax64_memcpy/candidate.asm"
...
try {
    ...
    if ($text -notmatch "global memcpy") {
        throw "emit output missing global memcpy"
    }
