...
$source = Join-Path $hlaxRoot "examples/interop/semasm-vaa/count_byte/count_byte.hla64"
$outAsm = Join-Path $vaaRoot "fixtures/ingest/hlax64_count_byte/candidate.asm"
...
try {
    ...
    if ($text -notmatch "global count_byte") {
        throw "emit output missing global count_byte"
    }
