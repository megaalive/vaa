...
<#
.SYNOPSIS
  Regenerate fixtures/ingest/hlax64_max_usize/candidate.asm from HlaX64.
...
$source = Join-Path $hlaxRoot "examples/interop/semasm-vaa/max_usize/max_usize.hla64"
$outAsm = Join-Path $vaaRoot "fixtures/ingest/hlax64_max_usize/candidate.asm"
...
try {
    ...
    if ($text -notmatch "global max_usize") {
        throw "emit output missing global max_usize"
    }
    ...
    if ($text -match "cmp qword \[" -and $text -match "\], qword \[") {
        throw "emit contains an invalid mem-mem cmp; load operands into registers first"
    }
