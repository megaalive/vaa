...
<#
.SYNOPSIS
  Regenerate fixtures/ingest/hlax64_memcmp/candidate.asm from HlaX64.
...
$source = Join-Path $hlaxRoot "examples/interop/semasm-vaa/memcmp/memcmp.hla64"
$outAsm = Join-Path $vaaRoot "fixtures/ingest/hlax64_memcmp/candidate.asm"
...
try {
    ...
    if ($text -notmatch "global memcmp") {
        throw "emit output missing global memcmp"
    }
