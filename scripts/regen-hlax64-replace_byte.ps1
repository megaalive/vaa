...
<#
.SYNOPSIS
  Regenerate fixtures/ingest/hlax64_replace_byte/candidate.asm from HlaX64.
...
$source = Join-Path $hlaxRoot "examples/interop/semasm-vaa/replace_byte/replace_byte.hla64"
$outAsm = Join-Path $vaaRoot "fixtures/ingest/hlax64_replace_byte/candidate.asm"
...
try {
    ...
    if ($text -notmatch "global replace_byte") {
        throw "emit output missing global replace_byte"
    }
