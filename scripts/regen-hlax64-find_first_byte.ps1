...
<#
.SYNOPSIS
  Regenerate fixtures/ingest/hlax64_find_first_byte/candidate.asm from HlaX64.
...
$source = Join-Path $hlaxRoot "examples/interop/semasm-vaa/find_first_byte/find_first_byte.hla64"
$outAsm = Join-Path $vaaRoot "fixtures/ingest/hlax64_find_first_byte/candidate.asm"
...
try {
    ...
    if ($text -notmatch "global find_first_byte") {
        throw "emit output missing global find_first_byte"
    }
