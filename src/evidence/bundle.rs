...
pub fn verify_bundle(bundle_dir: &Path) -> Result<SealEnvelope, SealError> {
    ...
    if locked.task().task_id != envelope.provenance.task_id {
        return Err(SealError::Bundle(
            "task.task_id != provenance.task_id".into(),
        ));
    }
    if locked.task().target != envelope.acceptance.target {
        return Err(SealError::Bundle("task.target != acceptance.target".into()));
    }
...
mod tests {
    ...
    #[test]
    fn verify_bundle_rejects_target_label_mismatch() {
        let task_path =
            PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("fixtures/tasks/sum_i64.vaa.toml");
        let task = load_locked_task(&task_path).unwrap();
        let task_bytes = fs::read(&task_path).unwrap();
        ...
        envelope.acceptance.target = "x86_64-pc-windows-msvc".into();
        envelope.acceptance_digest =
            crate::evidence::seal::acceptance_digest_of(&envelope.acceptance);
        envelope.envelope_digest =
            crate::evidence::seal::envelope_digest_of(&envelope.acceptance, &envelope.provenance);
        fs::write(
            dir.join(BUNDLE_EVIDENCE),
            serde_json::to_string_pretty(&report).unwrap(),
        )
        .unwrap();
        fs::write(
            dir.join(BUNDLE_SEAL),
            serde_json::to_string_pretty(&envelope).unwrap(),
        )
        .unwrap();

        let err = verify_bundle(&dir).expect_err("target label");
        match err {
            SealError::Bundle(msg) => assert!(msg.contains("task.target != acceptance.target")),
            other => panic!("{other:?}"),
        }
        let _ = fs::remove_dir_all(&dir);
    }
    ...
    #[test]
    fn verify_bundle_rejects_task_id_label_mismatch() {
        let task_path =
            PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("fixtures/tasks/sum_i64.vaa.toml");
        let task = load_locked_task(&task_path).unwrap();
        let task_bytes = fs::read(&task_path).unwrap();
        ...
        report.task_id = "tampered-id".into();
        envelope.provenance.task_id = "tampered-id".into();
        envelope.envelope_digest =
            crate::evidence::seal::envelope_digest_of(&envelope.acceptance, &envelope.provenance);
        fs::write(
            dir.join(BUNDLE_EVIDENCE),
        ...
        let err = verify_bundle(&dir).expect_err("task_id label");
        match err {
            SealError::Bundle(msg) => assert!(msg.contains("task.task_id != provenance.task_id")),
            other => panic!("{other:?}"),
        }
        let _ = fs::remove_dir_all(&dir);
    }
