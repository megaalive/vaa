...
`00_write_broken.asm` writes one byte past the declared `buffer[0..length]`
region regardless of `value`, which fails the SemASM behavioral oracle
(`builtin.buffer.memset`) and is reported `Violated`, mirroring the `memcmp` /
`find_first_byte` / `replace_byte` adversarial seeds.
