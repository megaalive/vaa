...
#[cfg(target_os = "linux")]
#[ignore = "requires `semasm` on PATH, Linux toolchain, and SemASM --allow-execution"]
fn gate2_verify_replace_byte_linux_verified() {
    ...
    let value: serde_json::Value = match serde_json::from_str(&stdout) {
        Ok(v) => v,
        Err(error) => {
            eprintln!("skipping Gate-2 Linux replace_byte: no evidence JSON ({error})\n{stdout}");
            return;
        }
    };

    if value["doctor"]["status"] == "Unavailable" {
        eprintln!("skipping Gate-2 Linux replace_byte: SemASM unavailable");
        return;
    }

    assert_gate2_verified_or_under_preconditions(&value, "Gate-2 Linux replace_byte");
    let raw_json = value["verify_report"]["raw_json"]
        .as_str()
        .expect("verify_report.raw_json");
    let raw: serde_json::Value = serde_json::from_str(raw_json).expect("raw_json parse");
...
#[test]
#[ignore = "requires `semasm` on PATH, Win64 toolchain, and SemASM --allow-execution"]
fn gate2_verify_replace_byte_win64_verified() {
    ...
    let value: serde_json::Value = match serde_json::from_str(&stdout) {
        Ok(v) => v,
        Err(error) => {
            eprintln!("skipping Gate-2 replace_byte: no evidence JSON ({error})\n{stdout}");
            return;
        }
    };

    if value["doctor"]["status"] == "Unavailable" {
        eprintln!("skipping Gate-2 replace_byte: SemASM unavailable");
        return;
    }

    assert_gate2_verified_or_under_preconditions(&value, "Gate-2 replace_byte Win64");
    let raw_json = value["verify_report"]["raw_json"]
        .as_str()
        .expect("verify_report.raw_json");
    let raw: serde_json::Value = serde_json::from_str(raw_json).expect("raw_json parse");
...
#[cfg(target_os = "linux")]
#[ignore = "requires `semasm` on PATH, Linux toolchain, and SemASM --allow-execution"]
fn gate2_verify_memset_linux_verified() {
    ...
    let value: serde_json::Value = match serde_json::from_str(&stdout) {
        Ok(v) => v,
        Err(error) => {
            eprintln!("skipping Gate-2 Linux memset: no evidence JSON ({error})\n{stdout}");
            return;
        }
    };

    if value["doctor"]["status"] == "Unavailable" {
        eprintln!("skipping Gate-2 Linux memset: SemASM unavailable");
        return;
    }
...
#[test]
#[ignore = "requires `semasm` on PATH, Win64 toolchain, and SemASM --allow-execution"]
fn gate2_verify_memset_win64_verified() {
    ...
    let value: serde_json::Value = match serde_json::from_str(&stdout) {
        Ok(v) => v,
        Err(error) => {
            eprintln!("skipping Gate-2 memset: no evidence JSON ({error})\n{stdout}");
            return;
        }
    };

    if value["doctor"]["status"] == "Unavailable" {
        eprintln!("skipping Gate-2 memset: SemASM unavailable");
        return;
    }
...
#[cfg(target_os = "linux")]
#[ignore = "requires `semasm` on PATH, Linux toolchain, and SemASM --allow-execution"]
fn gate2_verify_memcmp_linux_verified() {
    ...
    let value: serde_json::Value = match serde_json::from_str(&stdout) {
        Ok(v) => v,
        Err(error) => {
            eprintln!("skipping Gate-2 Linux memcmp: no evidence JSON ({error})\n{stdout}");
            return;
        }
    };

    if value["doctor"]["status"] == "Unavailable" {
        eprintln!("skipping Gate-2 Linux memcmp: SemASM unavailable");
        return;
    }

    assert_gate2_verified_or_under_preconditions(&value, "Gate-2 Linux memcmp");
}

#[test]
#[ignore = "requires `semasm` on PATH, Win64 toolchain, and SemASM --allow-execution"]
fn gate2_verify_memcmp_win64_verified() {
    ...
    let value: serde_json::Value = match serde_json::from_str(&stdout) {
        Ok(v) => v,
        Err(error) => {
            eprintln!("skipping Gate-2 memcmp: no evidence JSON ({error})\n{stdout}");
            return;
        }
    };

    if value["doctor"]["status"] == "Unavailable" {
        eprintln!("skipping Gate-2 memcmp: SemASM unavailable");
        return;
    }

    assert_gate2_verified_or_under_preconditions(&value, "Gate-2 memcmp Win64");
    let raw_json = value["verify_report"]["raw_json"]
        .as_str()
        .expect("verify_report.raw_json");
    let raw: serde_json::Value = serde_json::from_str(raw_json).expect("raw_json parse");
...
#[test]
#[ignore = "requires `semasm` on PATH, Win64 toolchain, and SemASM --allow-execution"]
fn gate2_verify_hlax64_find_last_byte_win64_verified() {
    ...
    let value: serde_json::Value = match serde_json::from_str(&stdout) {
        ...
        Err(error) => {
            eprintln!(
                "skipping Gate-2 hlax64 find_last_byte: no evidence JSON ({error})\n{stdout}"
            );
            return;
        }
    };
    ...
    if value["doctor"]["status"] == "Unavailable" {
        eprintln!("skipping Gate-2 hlax64 find_last_byte: SemASM unavailable");
        return;
    }
...
#[test]
#[ignore = "requires `semasm` on PATH, Win64 toolchain, and SemASM --allow-execution"]
fn gate2_verify_hlax64_memcmp_win64_verified() {
    ...
    let value: serde_json::Value = match serde_json::from_str(&stdout) {
        Ok(v) => v,
        Err(error) => {
            eprintln!("skipping Gate-2 hlax64 memcmp: no evidence JSON ({error})\n{stdout}");
            return;
        }
    };

    if value["doctor"]["status"] == "Unavailable" {
        eprintln!("skipping Gate-2 hlax64 memcmp: SemASM unavailable");
        return;
    }

    assert_gate2_verified_or_under_preconditions(&value, "Gate-2 hlax64 memcmp");
    let raw_json = value["verify_report"]["raw_json"]
        .as_str()
        .expect("verify_report.raw_json");
    let raw: serde_json::Value = serde_json::from_str(raw_json).expect("raw_json parse");
...
fn gate2_verify_hlax64_replace_byte_win64_verified() {
    ...
    assert_gate2_verified_or_under_preconditions(&value, "Gate-2 hlax64 replace_byte");
    let raw_json = value["verify_report"]["raw_json"]
        .as_str()
        .expect("verify_report.raw_json");
    let raw: serde_json::Value = serde_json::from_str(raw_json).expect("raw_json parse");
...
#[test]
#[ignore = "requires `semasm` on PATH, Win64 toolchain, and SemASM --allow-execution"]
fn gate2_verify_hlax64_count_byte_win64_verified() {
    ...
    let value: serde_json::Value = match serde_json::from_str(&stdout) {
        Ok(v) => v,
        Err(error) => {
            eprintln!("skipping Gate-2 hlax64 count_byte: no evidence JSON ({error})\n{stdout}");
            return;
        }
    };

    if value["doctor"]["status"] == "Unavailable" {
        eprintln!("skipping Gate-2 hlax64 count_byte: SemASM unavailable");
        return;
    }
...
#[test]
#[ignore = "requires `semasm` on PATH, Win64 toolchain, and SemASM --allow-execution"]
fn gate2_verify_hlax64_find_first_byte_win64_verified() {
    ...
    let value: serde_json::Value = match serde_json::from_str(&stdout) {
        ...
        Err(error) => {
            eprintln!(
                "skipping Gate-2 hlax64 find_first_byte: no evidence JSON ({error})\n{stdout}"
            );
            return;
        }
    };
    ...
    if value["doctor"]["status"] == "Unavailable" {
        eprintln!("skipping Gate-2 hlax64 find_first_byte: SemASM unavailable");
        return;
    }
...
#[test]
#[ignore = "requires `semasm` on PATH, Win64 toolchain, and SemASM --allow-execution"]
fn gate2_verify_hlax64_memset_win64_verified() {
    ...
    let value: serde_json::Value = match serde_json::from_str(&stdout) {
        Ok(v) => v,
        Err(error) => {
            eprintln!("skipping Gate-2 hlax64 memset: no evidence JSON ({error})\n{stdout}");
            return;
        }
    };

    if value["doctor"]["status"] == "Unavailable" {
        eprintln!("skipping Gate-2 hlax64 memset: SemASM unavailable");
        return;
    }
...
#[test]
#[ignore = "requires `semasm` on PATH, Win64 toolchain, and SemASM --allow-execution"]
fn gate2_verify_hlax64_memcpy_win64_verified() {
    ...
    let value: serde_json::Value = match serde_json::from_str(&stdout) {
        Ok(v) => v,
        Err(error) => {
            eprintln!("skipping Gate-2 hlax64 memcpy: no evidence JSON ({error})\n{stdout}");
            return;
        }
    };

    if value["doctor"]["status"] == "Unavailable" {
        eprintln!("skipping Gate-2 hlax64 memcpy: SemASM unavailable");
        return;
    }
...
fn gate2_search_ingest_allow_execution_verified() {
    ...
    let run_base = root().join("target/vaa-gate2-search-ingest-verified");
    let _ = std::fs::remove_dir_all(&run_base);
    std::fs::create_dir_all(&run_base).unwrap();
    ...
    let output = Command::new(vaa_bin())
        ...
        .output()
        .expect("run vaa search --ingest --allow-execution");
    ...
    assert!(
        ...
        "search --ingest --allow-execution failed: {:?}\n{stdout}\n{stderr}",
        output.status
    );
    assert!(
        stdout.contains("verified=true"),
        "Gate-2 search must report verified=true: {stdout}"
    );
    assert!(
...
fn gate2_search_ingest_memcmp_allow_execution_verified() {
    ...
    assert!(
        stdout.contains("reason=verified")
            ...
            || stdout.contains("\"stopped_reason\":\"verified\"")
            || stdout.contains("\"stopped_reason\":\"verified_under_preconditions\""),
        "Gate-2 memcmp search must stop on SemASM verified(_under_preconditions): {stdout}"
    );
    let stdout_lower = stdout.to_lowercase();
    assert!(
        !stdout_lower.contains("cryptopt verified"),
...
#[test]
#[ignore = "requires semasm + Win64 + allow-execution"]
fn gate2_search_ingest_find_first_allow_execution_verified() {
    ...
    let run_base = root().join("target/vaa-gate2-search-ingest-find-first-verified");
    let _ = std::fs::remove_dir_all(&run_base);
    std::fs::create_dir_all(&run_base).unwrap();
    ...
    let output = Command::new(vaa_bin())
        ...
        .output()
        .expect("run vaa search --ingest --allow-execution find_first");
    ...
    assert!(
        output.status.success(),
        "find_first search --ingest --allow-execution failed: {:?}\\n{stdout}\\n{stderr}",
        output.status
    );
    assert!(
        stdout.contains("verified=true"),
        "Gate-2 find_first search must report verified=true: {stdout}"
    );
    assert!(
        stdout.contains("reason=verified")
            || stdout.contains("stopped_reason=verified")
...
#[test]
#[ignore = "requires semasm + Win64 + allow-execution"]
fn gate2_search_ingest_replace_byte_allow_execution_verified() {
    ...
    let run_base = root().join("target/vaa-gate2-search-ingest-replace-byte-verified");
    let _ = std::fs::remove_dir_all(&run_base);
    std::fs::create_dir_all(&run_base).unwrap();
    ...
    let output = Command::new(vaa_bin())
        ...
        .output()
        .expect("run vaa search --ingest --allow-execution replace_byte");
    ...
    assert!(
        output.status.success(),
        "replace_byte search --ingest --allow-execution failed: {:?}\n{stdout}\n{stderr}",
        output.status
    );
    assert!(
        stdout.contains("verified=true"),
        "Gate-2 replace_byte search must report verified=true: {stdout}"
    );
    assert!(
        stdout.contains("reason=verified")
            || stdout.contains("reason=verified_under_preconditions")
            ...
            || stdout.contains("\"stopped_reason\":\"verified\"")
            || stdout.contains("\"stopped_reason\":\"verified_under_preconditions\""),
        "Gate-2 replace_byte search must stop on SemASM verified(_under_preconditions): {stdout}"
    );
    let stdout_lower = stdout.to_lowercase();
    assert!(
        !stdout_lower.contains("cryptopt verified"),
...
#[test]
#[ignore = "requires semasm + Win64 + allow-execution"]
fn gate2_search_ingest_memset_allow_execution_verified() {
    ...
    let run_base = root().join("target/vaa-gate2-search-ingest-memset-verified");
    let _ = std::fs::remove_dir_all(&run_base);
    std::fs::create_dir_all(&run_base).unwrap();
    ...
    let output = Command::new(vaa_bin())
        ...
        .output()
        .expect("run vaa search --ingest --allow-execution memset");
    ...
    assert!(
        output.status.success(),
        "memset search --ingest --allow-execution failed: {:?}\n{stdout}\n{stderr}",
        output.status
    );
    assert!(
        stdout.contains("verified=true"),
        "Gate-2 memset search must report verified=true: {stdout}"
    );
    assert!(
        stdout.contains("reason=verified")
            || stdout.contains("reason=verified_under_preconditions")
...
#[test]
#[ignore = "requires semasm + Win64 + allow-execution"]
fn gate2_search_ingest_memcpy_allow_execution_verified() {
    ...
    let run_base = root().join("target/vaa-gate2-search-ingest-memcpy-verified");
    let _ = std::fs::remove_dir_all(&run_base);
    std::fs::create_dir_all(&run_base).unwrap();
    ...
    let output = Command::new(vaa_bin())
        ...
        .output()
        .expect("run vaa search --ingest --allow-execution memcpy");
    ...
    assert!(
        output.status.success(),
        "memcpy search --ingest --allow-execution failed: {:?}\n{stdout}\n{stderr}",
        output.status
    );
    assert!(
        stdout.contains("verified=true"),
        "Gate-2 memcpy search must report verified=true: {stdout}"
    );
    assert!(
        stdout.contains("reason=verified")
            || stdout.contains("reason=verified_under_preconditions")
