...
#[test]
fn hlax64_memcmp_candidate_is_framed_win64() {
    let src = include_str!("../fixtures/ingest/hlax64_memcmp/candidate.asm");
    assert!(
        src.contains("global memcmp"),
        "HlaX64 memcmp leaf must export the SemASM symbol"
    );
    assert!(
        !src.contains("mov rax, -1") && !src.contains("mov rax,-1"),
        "memcmp emit must avoid model-hostile mov rax,-1"
    );
    assert!(
        src.contains("push rbp"),
        "HlaX64 framed leaf must open a frame"
...
#[test]
fn hlax64_replace_byte_candidate_is_framed_win64() {
    let src = include_str!("../fixtures/ingest/hlax64_replace_byte/candidate.asm");
    assert!(
        src.contains("global replace_byte"),
        "HlaX64 replace_byte leaf must export the SemASM symbol"
    );
    assert!(
        !src.contains("mov rax, -1") && !src.contains("mov rax,-1"),
        "replace_byte emit must avoid model-hostile mov rax,-1"
    );
    assert!(
        src.contains("push rbp"),
        "HlaX64 framed leaf must open a frame"
...
#[test]
fn hlax64_count_byte_candidate_is_framed_win64() {
    let src = include_str!("../fixtures/ingest/hlax64_count_byte/candidate.asm");
    assert!(
        src.contains("global count_byte"),
        "HlaX64 count_byte leaf must export the SemASM symbol"
    );
    assert!(
        !src.contains("mov rax, -1") && !src.contains("mov rax,-1"),
        "count_byte emit must avoid model-hostile mov rax,-1"
    );
    assert!(
        src.contains("push rbp"),
        "HlaX64 framed leaf must open a frame"
...
#[test]
fn hlax64_find_first_byte_candidate_is_framed_win64() {
    let src = include_str!("../fixtures/ingest/hlax64_find_first_byte/candidate.asm");
    assert!(
        src.contains("global find_first_byte"),
        "HlaX64 find_first_byte leaf must export the SemASM symbol"
    );
    assert!(
        !src.contains("mov rax, -1") && !src.contains("mov rax,-1"),
        "find_first_byte emit must avoid model-hostile mov rax,-1"
    );
    assert!(
        src.contains("push rbp"),
        "HlaX64 framed leaf must open a frame"
...
#[test]
fn hlax64_memset_candidate_is_framed_win64() {
    let src = include_str!("../fixtures/ingest/hlax64_memset/candidate.asm");
    assert!(
        src.contains("global memset"),
        "HlaX64 memset leaf must export the SemASM symbol"
    );
    assert!(
        !src.contains("mov rax, -1") && !src.contains("mov rax,-1"),
        "memset emit must avoid model-hostile mov rax,-1"
    );
    assert!(
        src.contains("push rbp"),
        "HlaX64 framed leaf must open a frame"
...
#[test]
fn hlax64_min_usize_candidate_is_framed_win64() {
    let src = include_str!("../fixtures/ingest/hlax64_min_usize/candidate.asm");
    assert!(
        src.contains("global min_usize"),
        "HlaX64 min_usize leaf must export the SemASM symbol"
    );
    assert!(
        src.contains("push rbp"),
        "HlaX64 framed leaf must open a frame"
    ...
    assert!(
        !(src.contains("cmp qword [") && src.contains("], qword [")),
        "HlaX64 pure-int leaf must not lower to an invalid mem-mem cmp"
    );
}

#[test]
fn hlax64_max_usize_candidate_is_framed_win64() {
    let src = include_str!("../fixtures/ingest/hlax64_max_usize/candidate.asm");
    assert!(
        src.contains("global max_usize"),
        "HlaX64 max_usize leaf must export the SemASM symbol"
    );
    assert!(
        src.contains("push rbp"),
        "HlaX64 framed leaf must open a frame"
...
#[cfg(target_os = "linux")]
#[ignore = "requires `semasm` on PATH and a Linux assemble/link toolchain"]
fn gate1_verify_memset_linux_incomplete() {
    let task = root().join("fixtures/semasm/memset/memset_linux.vaa.toml");
    let source = root().join("fixtures/semasm/memset/memset_linux.asm");
    let contract = root().join("fixtures/semasm/memset/memset.sem.toml");
    ...
    assert_eq!(
        value["final_status"], "Incomplete",
        "Gate-1 Linux memset expects Incomplete: {value}"
    );
    assert_eq!(value["verify_report"]["raw_status"], "execution_denied");
    let raw_json = value["verify_report"]["raw_json"]
        .as_str()
        .expect("verify_report.raw_json");
    let raw: serde_json::Value = serde_json::from_str(raw_json).expect("raw_json parse");
    assert_eq!(raw["behavior_oracle"]["id"], "builtin.buffer.memset");
    assert_eq!(raw["behavior_oracle"]["version"], 1);
}

#[test]
#[ignore = "requires `semasm` on PATH and a Win64 assemble/link toolchain"]
fn gate1_verify_memset_win64_incomplete() {
    let task = root().join("fixtures/semasm/memset/memset.vaa.toml");
    let source = root().join("fixtures/semasm/memset/memset_win64.asm");
    let contract = root().join("fixtures/semasm/memset/memset.sem.toml");
    ...
    assert_eq!(
        value["final_status"], "Incomplete",
        "Gate-1 memset expects Incomplete: {value}"
    );
    assert_eq!(value["verify_report"]["raw_status"], "execution_denied");
    let raw_json = value["verify_report"]["raw_json"]
        .as_str()
...
#[cfg(target_os = "linux")]
#[ignore = "requires `semasm` on PATH and a Linux assemble/link toolchain"]
fn gate1_verify_memcpy_linux_incomplete() {
    ...
    assert_eq!(
        value["final_status"], "Incomplete",
        "Gate-1 Linux memcpy expects Incomplete: {value}"
    );
    assert_eq!(value["verify_report"]["raw_status"], "execution_denied");
    let raw_json = value["verify_report"]["raw_json"]
        .as_str()
...
#[test]
#[ignore = "requires `semasm` on PATH and a Win64 assemble/link toolchain"]
fn gate1_verify_memcpy_win64_incomplete() {
    ...
    assert_eq!(
        value["final_status"], "Incomplete",
        "Gate-1 memcpy expects Incomplete: {value}"
    );
    assert_eq!(value["verify_report"]["raw_status"], "execution_denied");
    let raw_json = value["verify_report"]["raw_json"]
        .as_str()
...
#[test]
#[ignore = "requires `semasm` on PATH and a Win64 assemble/link toolchain"]
fn gate1_ingest_hlax64_memcmp_verify_chain() {
    let task = root().join("fixtures/ingest/hlax64_memcmp/memcmp.vaa.toml");
    let source = root().join("fixtures/ingest/hlax64_memcmp/candidate.asm");
    let contract = root().join("fixtures/ingest/hlax64_memcmp/memcmp.sem.toml");
    let run_base = root().join("target/vaa-gate1-hlax64-memcmp-runs");
    let _ = std::fs::remove_dir_all(&run_base);
    std::fs::create_dir_all(&run_base).unwrap();
    ...
    let output = Command::new(vaa_bin())
        ...
        .output()
        .expect("run vaa ingest hlax64 memcmp");
    ...
    assert!(
        output.status.success() || stdout.contains("Incomplete") || stdout.contains("final_status"),
        "hlax64 memcmp ingest failed: status={:?}\nstdout={stdout}\nstderr={stderr}",
        output.status
    );
    ...
    let run_dir = std::fs::read_dir(&run_base)
        ...
        .map(|e| e.path())
        .find(|p| p.is_dir())
        .expect("expected a run directory after hlax64 memcmp ingest");
...
#[test]
#[ignore = "requires `semasm` on PATH and a Win64 assemble/link toolchain"]
fn gate1_ingest_hlax64_replace_byte_verify_chain() {
    ...
    let run_base = root().join("target/vaa-gate1-hlax64-replace-byte-runs");
    let _ = std::fs::remove_dir_all(&run_base);
    std::fs::create_dir_all(&run_base).unwrap();
    ...
    let output = Command::new(vaa_bin())
        ...
        .output()
        .expect("run vaa ingest hlax64 replace_byte");
    ...
    assert!(
        output.status.success() || stdout.contains("Incomplete") || stdout.contains("final_status"),
        "hlax64 replace_byte ingest failed: status={:?}\nstdout={stdout}\nstderr={stderr}",
        output.status
    );
    ...
    let run_dir = std::fs::read_dir(&run_base)
        ...
        .map(|e| e.path())
        .find(|p| p.is_dir())
        .expect("expected a run directory after hlax64 replace_byte ingest");
...
#[test]
#[ignore = "requires `semasm` on PATH and a Win64 assemble/link toolchain"]
fn gate1_ingest_hlax64_count_byte_verify_chain() {
    let task = root().join("fixtures/ingest/hlax64_count_byte/count_byte.vaa.toml");
    let source = root().join("fixtures/ingest/hlax64_count_byte/candidate.asm");
    let contract = root().join("fixtures/ingest/hlax64_count_byte/count_byte.sem.toml");
    let run_base = root().join("target/vaa-gate1-hlax64-count-byte-runs");
    let _ = std::fs::remove_dir_all(&run_base);
    std::fs::create_dir_all(&run_base).unwrap();
    ...
    let output = Command::new(vaa_bin())
        ...
        .output()
        .expect("run vaa ingest hlax64 count_byte");
    ...
    assert!(
        output.status.success() || stdout.contains("Incomplete") || stdout.contains("final_status"),
        "hlax64 count_byte ingest failed: status={:?}\nstdout={stdout}\nstderr={stderr}",
        output.status
    );
    ...
    let run_dir = std::fs::read_dir(&run_base)
        ...
        .map(|e| e.path())
        .find(|p| p.is_dir())
        .expect("expected a run directory after hlax64 count_byte ingest");
...
#[test]
#[ignore = "requires `semasm` on PATH and a Win64 assemble/link toolchain"]
fn gate1_ingest_hlax64_find_first_byte_verify_chain() {
    let task = root().join("fixtures/ingest/hlax64_find_first_byte/find_first_byte.vaa.toml");
    let source = root().join("fixtures/ingest/hlax64_find_first_byte/candidate.asm");
    let contract = root().join("fixtures/ingest/hlax64_find_first_byte/find_first_byte.sem.toml");
    let run_base = root().join("target/vaa-gate1-hlax64-find-first-byte-runs");
    let _ = std::fs::remove_dir_all(&run_base);
    std::fs::create_dir_all(&run_base).unwrap();
    ...
    let output = Command::new(vaa_bin())
        ...
        .output()
        .expect("run vaa ingest hlax64 find_first_byte");
    ...
    assert!(
        output.status.success() || stdout.contains("Incomplete") || stdout.contains("final_status"),
        "hlax64 find_first_byte ingest failed: status={:?}\nstdout={stdout}\nstderr={stderr}",
        output.status
    );
    ...
    let run_dir = std::fs::read_dir(&run_base)
        ...
        .map(|e| e.path())
        .find(|p| p.is_dir())
        .expect("expected a run directory after hlax64 find_first_byte ingest");
...
#[test]
#[ignore = "requires `semasm` on PATH and a Win64 assemble/link toolchain"]
fn gate1_ingest_hlax64_memset_verify_chain() {
    let task = root().join("fixtures/ingest/hlax64_memset/memset.vaa.toml");
    let source = root().join("fixtures/ingest/hlax64_memset/candidate.asm");
    let contract = root().join("fixtures/ingest/hlax64_memset/memset.sem.toml");
    let run_base = root().join("target/vaa-gate1-hlax64-memset-runs");
    let _ = std::fs::remove_dir_all(&run_base);
    std::fs::create_dir_all(&run_base).unwrap();
    ...
    let output = Command::new(vaa_bin())
        ...
        .output()
        .expect("run vaa ingest hlax64 memset");
    ...
    assert!(
        output.status.success() || stdout.contains("Incomplete") || stdout.contains("final_status"),
        "hlax64 memset ingest failed: status={:?}\nstdout={stdout}\nstderr={stderr}",
        output.status
    );
...
#[test]
#[ignore = "requires `semasm` on PATH and a Win64 assemble/link toolchain"]
fn gate1_ingest_hlax64_min_usize_verify_chain() {
    ...
    let run_base = root().join("target/vaa-gate1-hlax64-min-usize-runs");
    let _ = std::fs::remove_dir_all(&run_base);
    std::fs::create_dir_all(&run_base).unwrap();
    ...
    let output = Command::new(vaa_bin())
        ...
        .output()
        .expect("run vaa ingest hlax64 min_usize");
    ...
    assert!(
        output.status.success() || stdout.contains("Incomplete") || stdout.contains("final_status"),
        "hlax64 min_usize ingest failed: status={:?}\nstdout={stdout}\nstderr={stderr}",
        output.status
    );
    ...
    let run_dir = std::fs::read_dir(&run_base)
        ...
        .map(|e| e.path())
        .find(|p| p.is_dir())
        .expect("expected a run directory after hlax64 min_usize ingest");
...
#[test]
#[ignore = "requires `semasm` on PATH and a Win64 assemble/link toolchain"]
fn gate1_ingest_hlax64_max_usize_verify_chain() {
    ...
    let run_base = root().join("target/vaa-gate1-hlax64-max-usize-runs");
    let _ = std::fs::remove_dir_all(&run_base);
    std::fs::create_dir_all(&run_base).unwrap();
    ...
    let output = Command::new(vaa_bin())
        ...
        .output()
        .expect("run vaa ingest hlax64 max_usize");
    ...
    assert!(
        output.status.success() || stdout.contains("Incomplete") || stdout.contains("final_status"),
        "hlax64 max_usize ingest failed: status={:?}\nstdout={stdout}\nstderr={stderr}",
        output.status
    );
    ...
    let run_dir = std::fs::read_dir(&run_base)
        ...
        .map(|e| e.path())
        .find(|p| p.is_dir())
        .expect("expected a run directory after hlax64 max_usize ingest");
...
#[ignore = "requires semasm on PATH and a Win64 assemble/link toolchain"]
#[allow(clippy::too_many_lines)]
fn gate1_search_ingest_find_first_nop_before_ret_stops_on_incomplete() {
    let task = root().join("fixtures/run/find_first_byte/find_first_byte.vaa.toml");
    let contract = root().join("fixtures/run/find_first_byte/find_first_byte.sem.toml");
    let seed = root().join("fixtures/run/find_first_byte/02_repaired.asm");
    let run_base = root().join("target/vaa-gate1-search-ingest-find-first-incomplete");
    let _ = std::fs::remove_dir_all(&run_base);
    std::fs::create_dir_all(&run_base).unwrap();
    ...
    let output = Command::new(vaa_bin())
        ...
        .output()
        .expect("run vaa search --ingest find_first");
    ...
    assert!(
        output.status.success(),
        "find_first search --ingest failed: {:?}\\n{stdout}\\n{stderr}",
        output.status
    );
    assert!(
        stdout.contains("verified=false"),
    ...
    let run_dir = std::fs::read_dir(&run_base)
        ...
        .map(|e| e.path())
        .find(|p| p.is_dir())
        .expect("expected find_first search-ingest run directory");
...
#[test]
#[ignore = "requires semasm on PATH and a Win64 assemble/link toolchain"]
fn gate1_search_ingest_find_first_skips_violated_budget() {
    let task = root().join("fixtures/run/find_first_byte/find_first_byte.vaa.toml");
    let contract = root().join("fixtures/run/find_first_byte/find_first_byte.sem.toml");
    let seed = root().join("fixtures/run/find_first_byte/00_write_broken.asm");
    let run_base = root().join("target/vaa-gate1-search-ingest-find-first-violated");
    let _ = std::fs::remove_dir_all(&run_base);
    std::fs::create_dir_all(&run_base).unwrap();
    ...
    let output = Command::new(vaa_bin())
        ...
        .output()
        .expect("run vaa search --ingest find_first violated seed");
    ...
    assert!(
        output.status.success(),
        "find_first search --ingest violated seed failed: {:?}\\n{stdout}\\n{stderr}",
        output.status
    );
    assert!(
        stdout.contains("verified=false"),
...
#[ignore = "requires semasm on PATH and a Win64 assemble/link toolchain"]
#[allow(clippy::too_many_lines)]
fn gate1_search_ingest_replace_byte_nop_before_ret_stops_on_incomplete() {
    let task = root().join("fixtures/run/replace_byte/replace_byte.vaa.toml");
    let contract = root().join("fixtures/run/replace_byte/replace_byte.sem.toml");
    let seed = root().join("fixtures/run/replace_byte/02_repaired.asm");
    let run_base = root().join("target/vaa-gate1-search-ingest-replace-byte-incomplete");
    let _ = std::fs::remove_dir_all(&run_base);
    std::fs::create_dir_all(&run_base).unwrap();
    ...
    let output = Command::new(vaa_bin())
        ...
        .output()
        .expect("run vaa search --ingest replace_byte");
    ...
    assert!(
        output.status.success(),
        "replace_byte search --ingest failed: {:?}\n{stdout}\n{stderr}",
        output.status
    );
    assert!(
        stdout.contains("verified=false"),
    ...
    let run_dir = std::fs::read_dir(&run_base)
        ...
        .map(|e| e.path())
        .find(|p| p.is_dir())
        .expect("expected replace_byte search-ingest run directory");
...
#[test]
#[ignore = "requires semasm on PATH and a Win64 assemble/link toolchain"]
fn gate1_search_ingest_replace_byte_skips_violated_budget() {
    let task = root().join("fixtures/run/replace_byte/replace_byte.vaa.toml");
    let contract = root().join("fixtures/run/replace_byte/replace_byte.sem.toml");
    let seed = root().join("fixtures/run/replace_byte/00_write_broken.asm");
    let run_base = root().join("target/vaa-gate1-search-ingest-replace-byte-violated");
    let _ = std::fs::remove_dir_all(&run_base);
    std::fs::create_dir_all(&run_base).unwrap();
    ...
    let output = Command::new(vaa_bin())
        ...
        .output()
        .expect("run vaa search --ingest replace_byte violated seed");
    ...
    assert!(
        output.status.success(),
        "replace_byte search --ingest violated seed failed: {:?}\n{stdout}\n{stderr}",
        output.status
    );
    assert!(
        stdout.contains("verified=false"),
...
#[ignore = "requires semasm on PATH and a Win64 assemble/link toolchain"]
#[allow(clippy::too_many_lines)]
fn gate1_search_ingest_memset_nop_before_ret_stops_on_incomplete() {
    let task = root().join("fixtures/run/memset/memset.vaa.toml");
    let contract = root().join("fixtures/run/memset/memset.sem.toml");
    let seed = root().join("fixtures/run/memset/02_repaired.asm");
    let run_base = root().join("target/vaa-gate1-search-ingest-memset-incomplete");
    let _ = std::fs::remove_dir_all(&run_base);
    std::fs::create_dir_all(&run_base).unwrap();
    ...
    let output = Command::new(vaa_bin())
        ...
        .output()
        .expect("run vaa search --ingest memset");
    ...
    let run_dir = std::fs::read_dir(&run_base)
        ...
        .map(|e| e.path())
        .find(|p| p.is_dir())
        .expect("expected memset search-ingest run directory");
...
#[test]
#[ignore = "requires semasm on PATH and a Win64 assemble/link toolchain"]
fn gate1_search_ingest_memset_skips_violated_budget() {
    let task = root().join("fixtures/run/memset/memset.vaa.toml");
    let contract = root().join("fixtures/run/memset/memset.sem.toml");
    let seed = root().join("fixtures/run/memset/00_write_broken.asm");
    let run_base = root().join("target/vaa-gate1-search-ingest-memset-violated");
    let _ = std::fs::remove_dir_all(&run_base);
    std::fs::create_dir_all(&run_base).unwrap();
    ...
    let output = Command::new(vaa_bin())
        ...
        .output()
        .expect("run vaa search --ingest memset violated seed");
    ...
    assert!(
        output.status.success(),
        "memset search --ingest violated seed failed: {:?}\n{stdout}\n{stderr}",
        output.status
    );
    assert!(
        stdout.contains("verified=false"),
...
#[ignore = "requires semasm on PATH and a Win64 assemble/link toolchain"]
#[allow(clippy::too_many_lines)]
fn gate1_search_ingest_memcpy_nop_before_ret_stops_on_incomplete() {
    let task = root().join("fixtures/run/memcpy/memcpy.vaa.toml");
    let contract = root().join("fixtures/run/memcpy/memcpy.sem.toml");
    let seed = root().join("fixtures/run/memcpy/02_repaired.asm");
    let run_base = root().join("target/vaa-gate1-search-ingest-memcpy-incomplete");
    let _ = std::fs::remove_dir_all(&run_base);
    std::fs::create_dir_all(&run_base).unwrap();
    ...
    let output = Command::new(vaa_bin())
        ...
        .output()
        .expect("run vaa search --ingest memcpy");
    ...
    let run_dir = std::fs::read_dir(&run_base)
        ...
        .map(|e| e.path())
        .find(|p| p.is_dir())
        .expect("expected memcpy search-ingest run directory");
...
#[test]
#[ignore = "requires semasm on PATH and a Win64 assemble/link toolchain"]
fn gate1_search_ingest_memcpy_skips_violated_budget() {
    let task = root().join("fixtures/run/memcpy/memcpy.vaa.toml");
    let contract = root().join("fixtures/run/memcpy/memcpy.sem.toml");
    let seed = root().join("fixtures/run/memcpy/00_write_broken.asm");
    let run_base = root().join("target/vaa-gate1-search-ingest-memcpy-violated");
    let _ = std::fs::remove_dir_all(&run_base);
    std::fs::create_dir_all(&run_base).unwrap();
    ...
    let output = Command::new(vaa_bin())
        ...
        .output()
        .expect("run vaa search --ingest memcpy violated seed");
    ...
    assert!(
        output.status.success(),
        "memcpy search --ingest violated seed failed: {:?}\n{stdout}\n{stderr}",
        output.status
    );
    assert!(
        stdout.contains("verified=false"),
