...
**G1–G5 closed.** SemASM tip `0f9cd1e` (memset equal atom + region_access
under_preconditions + Sei P0/Ra; report schema `0.5`). Gate pin tracks tip
([plan](V0_2_CONTROLLER_DEPTH_PLAN.md) Vd12); Gate `count_byte` (including
HlaX64) uses `leaf-pure-v1`; Gate `memcpy` / `memset` use
`memory-leaf-affine-v1`. SemASM **Co** + **Mm** + Tw/Ff/Ab + Sei (sample ≠
CFG/CFI, region-precise store, or formal ABI / memory-safety proof). Tag
**`v0.2.1`** remains the last release archive (`22d1543`). **Production** trust
root / hardware HSM / operated remote log as Gate default remain Horizon-locked.
